...
export async function POST(request: Request) {
  ...
  let profile
  try {
    profile = await scrapeYoutubeProfile(handle)
  } catch (err) {
    if (existing) {
      const { data: watchlistRows } = await supabase
        .from("watchlists").select("creator_id").eq("user_id", user.id).eq("creator_id", existing.id)
    ...
    const message = err instanceof Error ? err.message : "Erreur lors du scraping YouTube"
    console.error("[Scraping YouTube]", message)
