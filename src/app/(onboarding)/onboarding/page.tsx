...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    if (step === 2 && selectedNiche) fetchCreators(selectedNiche)
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // 2. Stripe Checkout avec 7 jours d'essai gratuit
      const res = await fetch("/api/stripe/checkout?from=onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ plan: selectedPlan, interval: "month" }),
      })
      ...
      // 3. Fallback → redirect direct vers /dashboard
  ...
  const steps = [
    { label: "Ta niche", number: 1 },
    { label: "Tes réseaux", number: 2 },
    { label: "Créateurs", number: 3 },
    { label: "Ton plan", number: 4 },
    { label: "C'est prêt", number: 5 },
  ]
      ...
      >
        <div className="mb-8 flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-brutify-text-secondary font-body">
            Configure ton espace en 30 secondes.
          </p>
        </div>
          ...
          {steps.map((s, i) => (
            <div key={s.number} className="flex items-center gap-2">
              <motion.div 
                ...
                className={cn(
                  "flex h-8 w-8 items-center justify-center rounded-full text-xs font-body font-bold transition-all duration-300",
                  step > i
                    ? "bg-brutify-gold text-brutify-bg shadow-[0_0_20px_rgba(255,171,0,0.4)]"
                    : step === i
                      ? "border-2 border-brutify-gold text-brutify-gold shadow-[0_0_30px_rgba(255,171,0,0.5)] scale-110"
                      : "border border-white/[0.1] text-brutify-text-muted"
                )}
              >
                {step > i ? <Check className="h-3.5 w-3.5" /> : s.number}
              </motion.div>
              {i < steps.length - 1 && (
                <div className={cn("h-[3px] w-8 transition-all duration-500 rounded-full", step > i ? "bg-gradient-to-r from-brutify-gold via-brutify-gold-light to-brutify-gold/50 shadow-[0_0_8px_rgba(255,171,0,0.3)]" : "bg-white/[0.06]")} />
              )}
            </div>
          ))}
        </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 4 && (
              <StepReady
                key="ready"
                niche={selectedNiche!}
                creatorsCount={selectedCreators.size}
...
function StepNiche({ selected, onSelect, onNext }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25, ease: "easeOut" }}>
      <h2 className="font-display text-2xl tracking-wider text-brutify-text-primary mb-2">
        Quelle est ta <span className="text-gold-gradient">niche</span> ?
      </h2>
      <p className="text-sm text-brutify-text-secondary font-body mb-6">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25, ease: "easeOut" }}>
      ...
      <div className="space-y-4">
        ...
        />
        <SocialInput
          icon={<TikTokIcon className="h-3.5 w-3.5" style={{ color: "#69C9D0" } as React.CSSProperties} />}
          label="TikTok"
          placeholder="ton.handle"
          value={tiktok}
        ...
        />
        <SocialInput
          icon={<Youtube className="h-3.5 w-3.5" style={{ color: "#FF0000" }} />}
          label="YouTube"
      ...
      <div className="mt-8 flex gap-3">
        <motion.button onClick={onBack} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          className="rounded-xl border border-white/[0.1] bg-white/[0.03] px-5 py-3 text-sm font-body font-semibold text-brutify-text-primary hover:bg-white/[0.06] transition-all duration-200"
        >
          Retour
        </motion.button>
        <motion.button onClick={onNext} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          className="flex-1 rounded-xl bg-gold-gradient px-5 py-3 text-sm font-body font-bold text-brutify-bg shadow-[0_0_40px_rgba(255,171,0,0.3)] hover:shadow-[0_0_60px_rgba(255,171,0,0.5)] transition-shadow duration-200 flex items-center justify-center gap-2"
        >
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25, ease: "easeOut" }}>
      ...
      <div className="mt-6 flex gap-3">
        ...
        <motion.button onClick={onNext} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          className="flex-1 rounded-xl bg-gold-gradient px-5 py-3 text-sm font-body font-bold text-brutify-bg shadow-[0_0_20px_rgba(255,171,0,0.15)] hover:shadow-[0_0_30px_rgba(255,171,0,0.25)] transition-shadow duration-200 flex items-center justify-center gap-2"
        >
          Continuer <ChevronRight className="h-4 w-4" />
        </motion.button>
      </div>
...
function StepPlanSelection({ selectedPlan, onSelect, onNext, onBack }: {
          ...
          <motion.button
            ...
            className={cn(
              ...
              selectedPlan === plan.key
                ? "border-brutify-gold/40 bg-brutify-gold/[0.12] shadow-[0_0_40px_rgba(255,171,0,0.3)] scale-105"
                : "border-white/[0.06] bg-white/[0.02] hover:border-white/[0.12]"
            )}
          >
            {plan.badge && (
              <div className="absolute -top-2 right-3">
                <span className="text-[9px] font-bold text-brutify-bg bg-brutify-gold rounded-full px-2 py-0.5 uppercase tracking-wide">
                  {plan.badge}
                </span>
              </div>
            )}
            <div className="flex items-center justify-between mb-2">
              ...
              <div className={cn("flex h-5 w-5 shrink-0 items-center justify-center rounded-md border transition-all duration-200",
                selectedPlan === plan.key ? "border-brutify-gold bg-brutify-gold" : "border-white/[0.15] bg-transparent"
              )}>
                {selectedPlan === plan.key && <Check className="h-3 w-3 text-brutify-bg" />}
              </div>
            </div>
            <p className="text-[10px] font-body text-brutify-text-muted mb-3">{plan.tagline}</p>
            <div className="flex items-baseline gap-1 mb-1">
              <span className="font-display text-2xl text-brutify-gold">{plan.price}</span>
            </div>
            <p className="text-[10px] font-body text-brutify-gold mb-3">{plan.bp}</p>
            {plan.trial && (
              <p className="text-[10px] font-body text-green-400 mb-3">{plan.trial} · Sans CB</p>
            )}
            <ul className="space-y-1.5">
              {plan.features.map((f) => (
                <li key={f} className="flex items-start gap-2 text-[11px] font-body text-brutify-text-secondary">
                  <Check className="h-3 w-3 text-brutify-gold shrink-0 mt-0.5" />
                  {f}
                </li>
              ))}
            </ul>
...
function StepReady({ niche, creatorsCount, instagramHandle, selectedPlan, finishing, onFinish, onBack }: {
  niche: string; creatorsCount: number; instagramHandle: string; selectedPlan: string
  finishing: boolean; onFinish: () => void; onBack: () => void
}) {
  const planLabels = { free: "Free (50 BP)", creator: "Creator Trial (500 BP × 7j)", growth: "Growth (2000 BP)", scale: "Scale (6000 BP)" }
  const summaryItems = [
    { label: "Niche", value: niche },
    { label: "Créateurs suivis", value: String(creatorsCount) },
    ...(instagramHandle ? [{ label: "Instagram", value: `@${instagramHandle.replace(/^@/, "")}` }] : []),
    { label: "Plan", value: planLabels[selectedPlan as keyof typeof planLabels] ?? selectedPlan },
  ]
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25, ease: "easeOut" }} className="text-center relative">
      {/* Confetti */}
      ...
      <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-brutify-gold/[0.1] border border-brutify-gold/20">
        <Sparkles className="h-8 w-8 text-brutify-gold" />
      </div>

      <h2 className="font-display text-2xl tracking-wider text-brutify-text-primary mb-2">
        Ton profil est <span className="text-gold-gradient">prêt</span> !
      </h2>
      <p className="text-sm text-brutify-text-secondary font-body mb-8">
        On a tout ce qu&apos;il faut pour personnaliser ton expérience.
      </p>

      <div className="mx-auto max-w-xs space-y-3 mb-8">
        {summaryItems.map((item) => (
          <div key={item.label} className="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3">
            <span className="text-sm text-brutify-text-secondary font-body">{item.label}</span>
            <span className="text-sm font-body font-semibold text-brutify-gold">{item.value}</span>
          </div>
        ))}
      </div>
